...
class StableVideoDiffusionPipelineControlNet(DiffusionPipeline):
    ...
    def __call__(
        ...
        controlnet_condition = (controlnet_condition + 1.0) / 2
        ...
        controlnet_condition_all = controlnet_condition
        latents_all = latents
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for batch, ind_start in enumerate(range(0, num_frames, frames_per_batch-overlap)):
                    self.scheduler._step_index = None
                    if ind_start + frames_per_batch >= num_frames:
                        ind_start = num_frames - 1 - frames_per_batch
                    latents = latents_all[:,ind_start:ind_start+frames_per_batch].contiguous()
                    controlnet_condition = controlnet_condition_all[:,ind_start:ind_start+frames_per_batch].contiguous()
